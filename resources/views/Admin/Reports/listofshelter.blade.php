...
<div class="row">
    <div class="col-sm">
        <div class="card">
            ...
            <form action="{{route('searchshelter.admin')}}" method="POST">
            ...
            <div class="card-body" id="printreport">
                <table id="datatable" class="table table-light table-hover">
                    ...
                    @foreach($shelters as $shelter)
                        <tr>
                            <td style="text-align:center">{{$shelter->id}}</td>
                            <td style="text-align:center">
                                <img src="{{asset('uploads/animal-shelter/profile/'.$shelter->profile)}}" width="70px" height="70px" alt="photo">
                            </td>
                            <td style="text-align:center">{{$shelter->shelter_name}}</td>
                            <td style="text-align:center">{{$shelter->email}}</td>
                            <td style="text-align:center">{{$shelter->founder_name}}</td>
                            <td style="text-align:center">{{$shelter->contact}}</td>
                            <td style="text-align:center">{{$shelter->address}}</td>
                            <td style="text-align:center">{{ \Carbon\Carbon::parse($shelter->created_at)->format('F d, Y')}}</td>
                            <td style="text-align:center">
                    ...
                    @endforeach
                    @if(empty($shelter))   
                        <h6 class="alert alert-danger">No Animal Shelters Registered</h6>
                    @endif
                    </tbody>
                </table>
            </div>
