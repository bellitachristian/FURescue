    ...
    <div class="col-xl-6 col-lg-7">
        <div class="card shadow mb-4">
            <div class="card-header">
                Edit {{ $LoggedUserInfo['shelter_name']}} Profile
            </div>
            <div class="card-body">
                <form action="{{url('/EditProfile/'.$shelter->id)}}"  method="POST" enctype="multipart/form-data">
                    @csrf 
                        <div class="form-group-row" style="display:flex"> 
                                <div class="col-sm"> 
                                    ...
                                    <label style="font-size:14px" >Shelter Name</label>    
                                    <input style="margin-bottom:2%" type="text" value="{{$shelter->shelter_name}}" required class="form-control form-control-sm" id="name" name="shelter_name">
                                    <span class="text-danger">@error('name'){{$message}}@enderror</span>
                                    
                                    <label style="font-size:14px" >Address</label> 
                                    <input style="margin-bottom:2%" type="text" id="" value="{{$shelter->address}}"  class="form-control form-control-sm" required id="name" name="address">
                                    
                                    <label style="font-size:14px" >Contact Person</label> 
                                    <input style="margin-bottom:2%" type="text" value="{{$shelter->founder_name}}"  class="form-control form-control-sm" required id="name" name="founder_name">
                                    
                                    <label style="font-size:14px" >Contact Number</label> 
                                    <input style="margin-bottom:2%" type="number" value="{{$shelter->contact}}"  class="form-control form-control-sm" required id="name" name="contact">
                                    ...
                                    <input  style="margin-bottom:2%" type="number" value="{{$shelter->pay_pal}}" required  class="form-control form-control-sm" id="name" name="pay_pal"> 
                                    <span class="text-danger">@error('paypal'){{$message}}@enderror</span>   
                                </div>  
                            </div>      
                        </div>
                        <div class="card-footer">
                            <a href="{{url('/Profile/'.$shelter->id)}}"><button class="btn btn-secondary" type="button">Cancel</button></a>
...
<div class="modal fade"  id="EditPass"  tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            ...
            <form action="{{url('/UpdatePassword/'.$shelter->id)}}" id="passchange" method="POST">
                @csrf
