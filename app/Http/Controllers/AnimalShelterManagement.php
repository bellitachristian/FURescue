...
class AnimalShelterManagement extends Controller
{
    ...
    function receipt(){
        ...
        $data =array(
            'LoggedUserInfo'=>AnimalShelter::where('id','=',session('LoggedUser'))->first(),
            'shelter'=>AnimalShelter::where('id','=',session('LoggedUser'))->first(),
            'receipts'=>Receipt::where('status','pending')->where('owner_id',$shelter->shelter_name)->where('process','none')->where('usertype_id',2)->get(),
            'count'=>Receipt::where('process','confirmed')->where('owner_id',$shelter->shelter_name)->where('usertype_id',2)->count(),
        );
